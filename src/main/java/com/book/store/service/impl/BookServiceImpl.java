...
import com.book.store.repository.BookIssueRepository;
import com.book.store.repository.BookRepository;
import com.book.store.repository.CategoryRepository;
import com.book.store.repository.LanguageRepository;
import com.book.store.repository.UserRepository;
import com.book.store.repository.WatchlistRepository;
import com.book.store.response.BookResponse;
import com.book.store.response.Data;
import com.book.store.response.FeedItems;
import com.book.store.response.Meta;
import com.book.store.response.Notifications;
...
public class BookServiceImpl implements BookService {
	...
	private Book mapBookRequest(BookForm bookForm, Book book, boolean isAddBook) {
		...
		} else if (bookForm.getCategoryId() == 0 && !StringUtils.isEmpty(bookForm.getCategory())) {
			Category category = new Category();
			category.setName(bookForm.getCategory());
		...
		} else if (bookForm.getLanguageId() == 0 && !StringUtils.isEmpty(bookForm.getLanguage())) {
			Language language = new Language();
			language.setName(bookForm.getLanguage());
